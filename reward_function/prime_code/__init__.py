...
def compute_score(completion, test_cases, import_prefix, continuous=False):
    ...
    try:
        ...
        try:
            res, metadata = apps_check_correctness(in_outs=test_cases, generation=solution, import_prefix=import_prefix, timeout=5, debug=False)
        ...
        if continuous:
            ...
            for test_case_id, test_case in enumerate(test_cases_list):
                res, metadata = apps_check_correctness(in_outs=test_case, generation=solution, import_prefix=import_prefix, timeout=10, debug=False)
                try:
